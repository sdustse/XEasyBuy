					...
					<c:if test="${ ebprodcate.epcParentId==0}">
						...
						<c:forEach var="ebprodcate1" items="${prodcates}">
							<c:if test="${ebprodcate1.epcParentId==ebprodcate.epcId}">
								<dd><a href="product_list.jsp"> ${ebprodcate1.epcName}</a></dd>
							</c:if>
						</c:forEach>
					</c:if>
				</c:forEach>
	...
	<div class="main">
		<div class="product-list">
			...
			<ul class="product clearfix">
				<c:forEach items="${prodlist}" var="eprod">
					<li>
						<dl>
							<c:url value="getAProd.prod" var="aprod">
								<c:param name="id" value="${eprod.epId}"></c:param>
							</c:url>
							<dt><a href="${aprod}" target="_blank"><img src="images/product/1.jpg" /></a></dt>
							<dd class="title"><a href="${aprod}" target="_blank">${eprod.epName}</a></dd>
